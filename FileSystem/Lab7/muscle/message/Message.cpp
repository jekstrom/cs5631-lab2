...
const Message         & GetEmptyMessage()    {return _messagePool.GetDefaultObject();}
...
const uint8 * Message :: FindFlatAux(const AbstractDataArray * ada, uint32 index, uint32 & retNumBytes) const
{
   const ByteBufferDataArray * bbda = dynamic_cast<const ByteBufferDataArray *>(ada);
   const ByteBuffer * buf = bbda ? dynamic_cast<ByteBuffer *>(bbda->ItemAt(index)()) : NULL;
   if (buf)
   {
