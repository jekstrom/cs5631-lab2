...
public:
    ...
    */
   String(const char * str = NULL, uint32 maxLen = MUSCLE_NO_LIMIT) : _buffer(NULL), _bufferLen(0), _length(0) {if (str) (void) SetCstr(str, maxLen); else _smallBuffer[0] = '\0';}
   ...
   /** This constructor sets this String to be a substring of the specified String.
     ...
     */
   String(const String & str, uint32 beginIndex, uint32 endIndex=MUSCLE_NO_LIMIT) : _buffer(NULL), _bufferLen(0), _length(0) {(void) SetFromString(str, beginIndex, endIndex);}

   /** Destructor. */
   ~String() {if (_buffer != _smallBuffer) muscleFree(_buffer);}
