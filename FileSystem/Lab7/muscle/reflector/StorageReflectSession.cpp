...
int StorageReflectSession :: FindNodesCallback(DataNode & node, void * userData)
{
   ...
   else return (data->_results.GetNumItems() == data->_maxResults) ? -1 : node.GetDepth();  // continue traversal as usual unless, we have reached our limit
}
...
GetMatchCount(DataNode & node, const Message * optData, int rootDepth) const
{
   ...
   int matchCount = 0;
   for (HashtableIterator<String, PathMatcherEntry> iter(GetEntries()); iter.HasData(); iter++) if (PathMatches(node, optData, iter.GetValue(), rootDepth)) matchCount++;
   return matchCount;
}
               ...
               if ((nextMatcher == NULL)||(nextMatcher->Match(nextChildName())))
               {
                  ...
                  if (depth == data._rootDepth+numClausesInParser-1)
                  {
                     if (matched == false)
                     {
                        ...
                        if (((GetEntries().GetNumItems() == 1)&&((data._useFilters == false)||(iter.GetValue().GetFilter()() == NULL)))||
