...
static ConstSocketRef CreateMuscleSocket(int socketType, uint32 createType)
{
   ...
   if (s >= 0)
   {
      ...
      if (ret())
      {
#if defined(MUSCLE_USE_IPV6)
         // If we're doing IPv4-mapped IPv6 addresses, we gotta tell Windows 7
         // that it's okay to use them, otherwise they won't work.  :^P
...
#if defined(MUSCLE_USE_IPV6) && defined(MUSCLE_ENABLE_MULTICAST_API)
