...
public:
   ...
   void operator++(int) 
   {
      if (_scratchValuesValid) UnsetScratchValues();
                          else _iterCookie = _owner ? _owner->GetSubsequentEntry(_iterCookie, _flags) : NULL;
...
private:
   ...
   const KeyType & GetConstScratchKey() const {return *(reinterpret_cast<const KeyType *>(_scratchKey._buf));}
   KeyType   & GetScratchKey() const {return *(reinterpret_cast<KeyType *>(_scratchKey._buf));}
   ValueType & GetScratchVal() const {return *(reinterpret_cast<ValueType *>(_scratchVal._buf));}

   void SetScratchValues(const KeyType & key, const ValueType & val)
   {
      if (_scratchValuesValid == false)
      {
         _scratchValuesValid = true;
         (void) new (&GetScratchKey()) KeyType(key);
         (void) new (&GetScratchVal()) ValueType(val);
      }
      else
      {
         GetScratchKey() = key;
   ...
   void UnsetScratchValues()
   {
      if (_scratchValuesValid)
      {
         GetScratchKey().~KeyType();
         GetScratchVal().~ValueType();
         _scratchValuesValid = false;
      }
   }
   ...
   void UpdateKeyAndValuePointers()
   {
      if (_scratchValuesValid)
      {
         _currentKey = &GetConstScratchKey();
         _currentVal = &GetScratchVal();
      }
      else if (_iterCookie)
      {
   ...
   uint32 _flags;
   bool _scratchValuesValid;  // set to true iff we are using our _scratchKey and _scratchVal, because the HashtableEntry we were pointing at got removed
   ...
   // These are declared as byte arrays rather than actual objects, so that except in the (rare) case where
   // our iterator gets bumped, we never to call the constructor/destructors of these objects.  In the commmon
   // case they are just that much more memory on the stack.
   mutable union {
      uint8 _buf[sizeof(KeyType)];
      uint64 _forceAlignment;   // a union must be aligned to the most restrictive of its members, so this ensures that _buf is at least 8-byte-aligned
   } _scratchKey;

   mutable union {
      uint8 _buf[sizeof(ValueType)];
      uint64 _forceAlignment;   // a union must be aligned to the most restrictive of its members, so this ensures that _buf is at least 8-byte-aligned
   } _scratchVal;
};
...
private:
   ...
   {
      RegisterIterator(&iter);
      iter._iterCookie = GetEntry(ComputeHash(startAt), startAt);
      iter.UpdateKeyAndValuePointers();
   }
...
public:
    ...
    *  @return B_NO_ERROR If the operation succeeded, B_ERROR if it failed (out of memory?)
    */
   status_t PutWithDefault(const KeyType & key) {return Put(key, this->_defaultValue);}
...
public:
   ...
   /** Copy Constructor.  */
   Hashtable(const Hashtable & rhs) : HashtableMid<KeyType,ValueType,HashFunctorType,Hashtable<KeyType,ValueType,HashFunctorType> >(rhs.GetNumAllocatedItemSlots(), false, NULL) {(void) CopyFrom(rhs);}

   /** Templated pseudo-copy-constructor:  Allows us to be instantiated as a copy of a similar table with different functor types. */
   template<class RHSHashFunctorType, class RHSSubclassType> Hashtable(const HashtableMid<KeyType,ValueType,RHSHashFunctorType,RHSSubclassType> & rhs) : HashtableMid<KeyType,ValueType,HashFunctorType,Hashtable<KeyType,ValueType,HashFunctorType> >(rhs.GetNumAllocatedItemSlots(), false, NULL) {(void) CopyFrom(rhs);}
...
public:
   ...
   /** Copy Constructor.  */
   OrderedKeysHashtable(const OrderedKeysHashtable & rhs) : HashtableMid<KeyType,ValueType,HashFunctorType,OrderedKeysHashtable<KeyType,ValueType,KeyCompareFunctorType,HashFunctorType> >(rhs.GetNumAllocatedItemSlots(), true, NULL) {(void) CopyFrom(rhs);}

   /** Templated pseudo-copy-constructor:  Allows us to be instantiated as a copy of a similar table with different functor types. */
   template<class RHSKeyCompareFunctorType, class RHSHashFunctorType> OrderedKeysHashtable(const HashtableMid<KeyType,ValueType,RHSKeyCompareFunctorType,RHSHashFunctorType> & rhs) : HashtableMid<KeyType,ValueType,HashFunctorType,OrderedKeysHashtable<KeyType,ValueType,KeyCompareFunctorType,HashFunctorType> >(rhs.GetNumAllocatedItemSlots(), NULL) {(void) CopyFrom(rhs);}
...
public:
   ...
   /** Copy Constructor. */
   OrderedValuesHashtable(const OrderedValuesHashtable & rhs) : HashtableMid<KeyType,ValueType,HashFunctorType,OrderedValuesHashtable<KeyType,ValueType,ValueCompareFunctorType,HashFunctorType> >(rhs.GetNumAllocatedItemSlots(), true, NULL) {(void) CopyFrom(rhs);}

   /** The iterator type that goes with this HashtableMid type */
   typedef HashtableIterator<KeyType,ValueType,HashFunctorType> IteratorType;

   /** Templated pseudo-copy-constructor:  Allows us to be instantiated as a copy of a similar table with different functor types. */
   template<class RHSValueCompareFunctorType, class RHSHashFunctorType> OrderedValuesHashtable(const HashtableMid<KeyType,ValueType,RHSValueCompareFunctorType,RHSHashFunctorType> & rhs) : HashtableMid<KeyType,ValueType,HashFunctorType,OrderedValuesHashtable<KeyType,ValueType,ValueCompareFunctorType,HashFunctorType> >(rhs.GetNumAllocatedItemSlots(), true, NULL) {(void) CopyFrom(rhs);}
...
{
   const HashtableEntry * e = GetEntry(ComputeHash(key), key);
   return e ? &e->_value : NULL;
}
...
HashtableBase<KeyType,ValueType,HashFunctorType>::RemoveEntry(HashtableEntry * e, ValueType * optSetValue)
{
   ...
   _count--;
   e->PushToFreeList(_defaultKey, GetDefaultValue(), _freeHead, _table);
...
HashtableBase<KeyType,ValueType,HashFunctorType>::MoveToPosition(const KeyType & moveMe, uint32 idx)
{
   ...
   if (e)
   {
           if (idx == 0)             MoveToFrontAux(e);
      else if (idx >= GetNumItems()) MoveToBackAux(e);
      else 
      {
         RemoveIterationEntry(e);
...
HashtableBase<KeyType,ValueType,HashFunctorType>::RemoveIterationEntry(HashtableEntry * e)
{
   ...
   while(next)
   {
      ...
      {
         if (next->_scratchValuesValid == false) next->SetScratchValues(e->_key, e->_value);
...
HashtableMid<KeyType,ValueType,HashFunctorType,SubclassType>::EnsureSize(uint32 requestedSize)
{
   ...
   // 3. Place all of our data into (biggerTable)
   {
      ...
      while(next)
      {
         ...
         if (hisClone)
         {
            ...
            while(nextIter)
            {
               if (GetIteratorNextCookie(*nextIter)   == next) SetIteratorScratchSpace(*nextIter, 0, hisClone);
               nextIter = GetIteratorNextIterator(*nextIter);
            }
         }
   ...
   // 6. Lastly, fix up our iterators to point to their new entries.
   {
      ...
      while(nextIter)
      {
         SetIteratorNextCookie(*nextIter, GetIteratorScratchSpace(*nextIter, 0));
         nextIter = GetIteratorNextIterator(*nextIter);
      }
   }
...
template <class KeyType, class ValueType, class HashFunctorType>
HashtableIterator<KeyType, ValueType, HashFunctorType>::HashtableIterator() : _iterCookie(NULL), _currentKey(NULL), _currentVal(NULL), _flags(0), _scratchValuesValid(false), _owner(NULL)
{
   // empty
}

template <class KeyType, class ValueType, class HashFunctorType>
HashtableIterator<KeyType, ValueType, HashFunctorType>::HashtableIterator(const HashtableIterator<KeyType, ValueType, HashFunctorType> & rhs) : _flags(0), _scratchValuesValid(false), _owner(NULL)
{
   *this = rhs;
}

template <class KeyType, class ValueType, class HashFunctorType>
HashtableIterator<KeyType, ValueType, HashFunctorType>::HashtableIterator(const HashtableBase<KeyType, ValueType, HashFunctorType> & table, uint32 flags) : _flags(flags), _scratchValuesValid(false), _owner(&table)
{
   table.InitializeIterator(*this);
}

template <class KeyType, class ValueType, class HashFunctorType>
HashtableIterator<KeyType, ValueType, HashFunctorType>::HashtableIterator(const HashtableBase<KeyType, ValueType, HashFunctorType> & table, const KeyType & startAt, uint32 flags) : _flags(flags), _scratchValuesValid(false), _owner(&table)
{
...
HashtableIterator<KeyType,ValueType,HashFunctorType>:: operator=(const HashtableIterator<KeyType,ValueType,HashFunctorType> & rhs)
{
   if (this != &rhs)
   {
      ...
      _owner = rhs._owner;
      if (rhs._scratchValuesValid) SetScratchValues(rhs.GetScratchKey(), rhs.GetScratchVal());
                              else UnsetScratchValues();
