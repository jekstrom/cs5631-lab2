...
int main(int argc, char ** argv)
{
   ...
   while(true)
   {
      ...
      FD_SET(fd, &readSet);
      if ((send)||(gw.HasBytesToOutput())) FD_SET(fd, &writeSet);
      ...
      if (select(fd+1, &readSet, &writeSet, NULL, NULL) < 0) LogTime(MUSCLE_LOG_CRITICALERROR, "bandwidthtester: select() failed!\n");
