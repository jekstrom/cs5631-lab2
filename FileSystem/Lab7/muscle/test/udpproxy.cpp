...
static status_t ReadIncomingData(const String & desc, DataIO & readIO, fd_set & readSet, Queue<ByteBufferRef> & outQ)
{
...
static status_t WriteOutgoingData(const String & desc, DataIO & writeIO, fd_set & writeSet, Queue<ByteBufferRef> & outQ, uint32 & writeIdx)
{
...
static status_t DoSession(const String aDesc, DataIO & aIO, const String & bDesc, DataIO & bIO)
{
   ...
   while(true)
   {
      ...
      FD_SET(aReadFD, &readSet);  
      FD_SET(bReadFD, &readSet);

      fd_set writeSet; FD_ZERO(&writeSet); 
      if (outgoingAData.HasItems()) FD_SET(aWriteFD, &writeSet); 
      if (outgoingBData.HasItems()) FD_SET(bWriteFD, &writeSet); 

      if (select(muscleMax(aReadFD, bReadFD, aWriteFD, bWriteFD)+1, &readSet, &writeSet, NULL, NULL) >= 0)
      {
         if (ReadIncomingData( aDesc, aIO, readSet,  outgoingBData)         != B_NO_ERROR) return B_ERROR;
         if (ReadIncomingData( bDesc, bIO, readSet,  outgoingAData)         != B_NO_ERROR) return B_ERROR;
         if (WriteOutgoingData(aDesc, aIO, writeSet, outgoingAData, aIndex) != B_NO_ERROR) return B_ERROR;
         if (WriteOutgoingData(bDesc, bIO, writeSet, outgoingBData, bIndex) != B_NO_ERROR) return B_ERROR;
      }
      else 
      {
