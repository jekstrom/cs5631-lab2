...
int main(int argc, char ** argv)
{
   ...
   while(1)
   {
      ...
      int maxFD = readFD;
      fd_set readSet;  FD_ZERO(&readSet);  FD_SET(readFD, &readSet);
      fd_set writeSet; FD_ZERO(&writeSet); if (gw.HasBytesToOutput()) {FD_SET(writeFD, &writeSet); maxFD = muscleMax(maxFD, writeFD);}
      ...
      if (spamInterval > 0)
      {
         ...
         if (now < nextSpamTime)
         {
            uint64 delay = nextSpamTime-now;
            Convert64ToTimeVal(delay, timeVal);
         }
      }

      if (select(maxFD+1, &readSet, &writeSet, NULL, (spamInterval>0)?&timeVal:NULL) < 0) LogTime(MUSCLE_LOG_CRITICALERROR, "testpackettunnel: select() failed!\n");

      bool reading = FD_ISSET(readFD,  &readSet);
      bool writing = FD_ISSET(writeFD, &writeSet);
